...
def go():
    # This is a very cheesy approach (and slow).  I think you could do better by implementing a connected
    # components count and checking to see which components your endpoints are in.  But this did work, so...
    start_x = 0
    start_y = 0
    finish_x = 70
    finish_y = 70
    ...
    nlines = 1024
    while ok:
        themap, lastpoint = read_input("input.txt",nlines)
        print(nlines, lastpoint)
        ok = dijkit(start_x,start_y,finish_x,finish_y,themap)
        nlines += 1
